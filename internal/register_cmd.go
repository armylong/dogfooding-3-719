...
func RegisterCmd(command command.BaseCommand) {
	...
	monitorCmd.Flags().BoolVar(&cmd.RefreshFlag, "refresh", false, "实时刷新显示")
	monitorCmd.Flags().IntVar(&cmd.IntervalFlag, "interval", 2, "刷新间隔（秒）")
	monitorCmd.Flags().StringVar(&cmd.SortFlag, "sort", "cpu", "排序方式（cpu/memory/pid）")
	monitorCmd.Flags().IntVar(&cmd.LimitFlag, "limit", 10, "显示数量限制")
